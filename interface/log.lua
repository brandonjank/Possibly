...
ActionLog:SetScript("OnMouseWheel", function(self, mouse)
  local top = #PossiblyEngine.actionLog.log - log_items
  ...
  PossiblyEngine.actionLog.update()
end)
...
PossiblyEngine.actionLog = {
  log = {}
}

PossiblyEngine.actionLog.insert = function(type, spell, spellIcon, target)
  if spellIcon then
    if PossiblyEngine.actionLog.log[1] and PossiblyEngine.actionLog.log[1]['event'] == type and PossiblyEngine.actionLog.log[1]['description'] == spell and PossiblyEngine.actionLog.log[1]['target'] == target then
      PossiblyEngine.actionLog.log[1]['count'] = PossiblyEngine.actionLog.log[1]['count'] + 1
    else
      table.insert(PossiblyEngine.actionLog.log, 1, {
        event = type,
        target = target or '',
        icon = spellIcon,
        description = spell,
      ...
      if delta > 0 and delta < #PossiblyEngine.actionLog.log - log_items then
        delta = delta + 1
      end
    end
  end
...
PossiblyEngine.actionLog.updateRow = function (row, a, b, c)
  ActionLogItem[row].itemA:SetText(a)
  ActionLogItem[row].itemB:SetText(b)
  ActionLogItem[row].itemC:SetText(c)
end

PossiblyEngine.actionLog.update = function ()
  local offset = 0
  for i = log_items, 1, -1 do
    offset = offset + 1
    local item = PossiblyEngine.actionLog.log[offset + delta]
    if not item then
      PossiblyEngine.actionLog.updateRow(i, '', '', '')
    else
      local target = stringLen(item.target) > 0 and ' @ (' .. item.target .. ')' or ''
      PossiblyEngine.actionLog.updateRow(i, item.event, 'x' .. item.count .. ' ' .. '|T' .. item.icon .. ':-1:-1:0:0|t' .. item.description .. target, item.time)
    end
  end
end

PossiblyEngine.timer.register("updateActionLog", function()
