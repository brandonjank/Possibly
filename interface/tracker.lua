...
local max_tracking = PossiblyEngine.config.read('ct_maxtracking', 10)
local tracking_height = 16
local tracking_width = 250
local colorR, colorB, colorG = HexToRGBPerc(PossiblyEngine.addonColor)
...
UnitTracker.toggle = function(load)
  if load then
    local toggleState = PossiblyEngine.config.read('ut_toggle')
    if toggleState then
      UnitTracker:Show()
    end
  else
    if UnitTracker:IsShown() then
      UnitTracker:Hide()
      PossiblyEngine.config.write('ut_toggle', false)
    else
      UnitTracker:Show()
      PossiblyEngine.config.write('ut_toggle', true)
    end
  end
end
...
local displayTTD = PossiblyEngine.config.read('ct_dttd', false)
local displayTotal = PossiblyEngine.config.read('ct_dtotal', false)
...
UnitTrackerToggle:SetScript("OnClick", function(self, button)
  ...
  PossiblyEngine.config.write('ct_dttd', displayTTD)
end)
...
UnitTrackerTotal:SetScript("OnClick", function(self, button)
  ...
  PossiblyEngine.config.write('ct_dtotal', displayTotal)
end)
    ...
    if unit.health and unit.health <= 1 then
      ...
      if currentRow <= max_tracking then
        ...
        if unit.health and unit.maxHealth then
          ...
          if displayTTD then
            if PossiblyEngine.module.combatTracker.enemy[guid]['ttd'] then
              seconds = PossiblyEngine.module.combatTracker.enemy[guid]['ttd']
        ...
        elseif unit.maxHealth then
          ...
          -- This happens often as we possibly are guessing the units HP
          totalHP = totalHP + unit.maxHealth
