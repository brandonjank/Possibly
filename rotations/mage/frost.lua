...
PossiblyEngine.library.register('magicBuff', {
  spells = function()
    for _, spell in ipairs(_ugly_buffs_table) do
      if PossiblyEngine.dsl.get('buff')('player', spell) then
        print(PossiblyEngine.dsl.get('buff')('player', spell))
        return spell
      end
    end
    return false
...
PossiblyEngine.rotation.register(64, {
