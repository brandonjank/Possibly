...
PossiblyEngine.parser.can_cast =  function(spell, unit, stopCasting)
  ...
  if turbo then
    ...
    if castEnds or channelEnds then
      ...
      if canCancel < (PossiblyEngine.lag*PossiblyEngine.turbo.modifier) then
        SpellStopCasting()
      end
    end
  end
  ...
  if not PossiblyEngine.faceroll.rolling then
  ...
  if spellBook == BOOKTYPE_PET then
    ...
    if PossiblyEngine.module.pet.casting then return false end
  ...
  if PossiblyEngine.module.player.casting == true and turbo == false then return false end
  if UnitChannelInfo("player") ~= nil then return false end
  return true
end

PossiblyEngine.parser.can_cast_queue =  function(spell)
...
PossiblyEngine.parser.nested = function(evaluationTable, event, target)
  local eval
  for _, evaluation in pairs(evaluationTable) do
    ...
    elseif evaluationType == "table" then
      eval = PossiblyEngine.parser.nested(evaluation, event, target) -- for the lulz
    elseif evaluationType == "string" then
      if string.sub(evaluation, 1, 1) == '@' then
        eval = PossiblyEngine.library.parse(event, evaluation, target)
      else
        eval = PossiblyEngine.dsl.parse(evaluation, event)
      end
...
PossiblyEngine.parser.table = function(spellTable, fallBackTarget)
  ...
  for _, arguments in pairs(spellTable) do
    ...
    if eventType == "string" then
      ...
      elseif string.sub(event, 1, 1) == '/' then
        eventType = "macro"
      elseif string.sub(event, 1, 1) == '@' then
        event = PossiblyEngine.library.parse(false, event, target)
      end
    end
    ...
    if eventType == "string" or eventType == "number" then
      if evaluationType == "string"  then
        evaluation = PossiblyEngine.dsl.parse(evaluation, event)
      elseif evaluationType == "table" then
        evaluation = PossiblyEngine.parser.nested(evaluation, event, target)
      elseif evaluationType == "function" then
        evaluation = evaluation()
      elseif evaluationType == "library" then
        evaluation = PossiblyEngine.library.parse(event, evaluation, target)
      elseif evaluationType == "nil" then
        evaluation = true
      end
    elseif eventType == "table" or eventType == "macro" or eventType == "item" then
      if evaluationType == "string"  then
        evaluation = PossiblyEngine.dsl.parse(evaluation, '')
      elseif evaluationType == "table" then
        evaluation = PossiblyEngine.parser.nested(evaluation, '', target)
      elseif evaluationType == "function" then
        evaluation = evaluation()
      elseif evaluationType == "library" then
        evaluation = PossiblyEngine.library.parse(event, evaluation, target)
    ...
    if PossiblyEngine.dsl.parsedTarget ~= nil then
      target = PossiblyEngine.dsl.parsedTarget
    end
    ...
    if eventType == "item" then
      local slot = event
      if PossiblyEngine.parser.items[slot] then
        slotId, itemTexture = GetInventorySlotInfo(PossiblyEngine.parser.items[slot])
        if slotId then
    ...
    if evaluation then
      ...
      elseif eventType == "item" then
        ...
        PossiblyEngine.actionLog.insert('Use Inventory Item', itemName, itemTexture, target)
        return false
      ...
      elseif event == "pause" then
        ...
        if PossiblyEngine.parser.can_cast(event, target, stopCasting) then
          return event, target
        end
      end
    end
